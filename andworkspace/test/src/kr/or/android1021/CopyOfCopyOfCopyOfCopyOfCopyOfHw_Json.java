...
public class CopyOfCopyOfCopyOfCopyOfCopyOfHw_Json extends TabActivity {
 ...
 ****************************************************************************/
		...
		class PlaceMapFragment extends FragmentActivity {
			@Override
				...
				try {
					...
					for (int i = 0; i < data.length(); i++) {
						...
						LinearLayout linear = new LinearLayout(
								CopyOfCopyOfCopyOfCopyOfCopyOfHw_Json.this);
	...
	****************************************************************************************/
		public void getLatlng(String input) {
			...
			try {
				addressList = gc.getFromLocationName(input, 1);
				if (addressList != null) {
					Address address = addressList.get(0);
					mlatitude = address.getLatitude();
					mlongitude = address.getLongitude();

					String slat = String.valueOf(mlatitude);
					String slng = String.valueOf(mlongitude);

					Log.d("lat :" + slat, "log :" + slng);
				}
			} catch (IOException e) {
				e.printStackTrace();
			}
